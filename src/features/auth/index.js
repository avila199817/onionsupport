/* =========================================================
   ...
   - Lecturas hot-path mediante el puerto runtime zero-copy de Core.
   - Escrituras normalizadas y single-write mediante Core runtimeState.
   ...
   - Sin Router, Toast, Store, Storage, fetch propio ni eventos internos.
   - Sin 2FA/MFA/OTP ni lógica de vistas.
========================================================= */
...
export const AUTH_VERSION =
  "auth.minimal.v7-runtime-state-port";
...
function currentGeneration() {
  return Number(sessionState.generation || 0);
}

function invalidateFlows() {
  sessionState.generation = currentGeneration() + 1;
  return sessionState.generation;
}

function flowIsCurrent(generation) {
  return generation === currentGeneration();
}

function syncActiveFlowCount() {
  sessionState.activeFlows = activeFlowControllers.size;
}

function createFlowAbort(externalSignal = null) {
  if (typeof AbortController === "undefined") {
    return { signal: externalSignal || undefined, cleanup: () => {}, abort: () => false };
  }
...
function abortActiveFlows() {
  for (const controller of [...activeFlowControllers]) {
    try {
      if (!controller.signal.aborted) controller.abort("auth-session-invalidated");
    } catch {
...
function withFlowSignal(options = {}, signal = undefined) {
...
function writeCoreState(patch = {}) {
  ...
  try {
    ...
    if (isFunction(AppCore?.setRuntimeState)) {
      AppCore.setRuntimeState(patch);
      return true;
    }
    if (isFunction(AppCore?.setState)) {
      AppCore.setState(patch, { raw: true, source: "auth" });
      return true;
    }
  } catch {}
  return false;
...
function installHttp() {
  try { Http.install?.(AppCore); } catch {}
  return Http;
}

function isRefreshableAuthError(error = null) {
  try { return Http.isRefreshableAuthError?.(error) === true; } catch { return false; }
}

function shouldClearSessionForAuthError(error = null) {
  try { return Http.shouldClearSessionForAuthError?.(error) === true; } catch { return false; }
}

function isHttpAuthError(error = null) {
  try { return Http.isAuthError?.(error) === true; } catch {
...
function roleOrUser(value = "") {
  return AppCore.normalizeRole(value) || "user";
}
...
function publicUser(user = null) {
  ...
  try {
    if (isFunction(AppCore?.publicUser)) return AppCore.publicUser(normalized);
  } catch {}
  return {
...
function buildUserHomePath(user = null) {
  ...
  try { return buildUserHomeRoute(slug) || ROOT_PATH; } catch {
...
function runtimeToken(state = coreState()) {
  return cleanToken(first(state?.token, state?.accessToken, state?.access_token, Http.getAccessToken?.(), ""));
}
...
function runtimeRoles(state = coreState(), user = runtimeUser(state)) {
...
function runtimeUserSlug(state = coreState(), user = runtimeUser(state)) {
  if (!user) return "";
  return normalizeUserSlug(first(state?.userSlug, getUserSlugFromUser(user), ""));
}

function runtimeHomePath(state = coreState(), user = runtimeUser(state)) {
  if (!user) return ROOT_PATH;
  return cleanText(first(state?.homePath, state?.defaultHome, buildUserHomePath(user), ROOT_PATH), ROOT_PATH);
}

function runtimePermissions(user = runtimeUser()) {
  if (!user) return [];
...
function runtimeHasRefreshToken(state = coreState(), session = runtimeSession(state)) {
  return Boolean(state?.hasRefreshToken === true || session?.persistent === true || session?.restoreOnBoot === true);
}

function cloneRuntimeUser(user = runtimeUser()) {
  if (!user) return null;
  return {
    ...user,
...
function cloneRuntimeSession(session = runtimeSession()) {
...
function readAuthContext() {
  ...
  const role = runtimeRole(state, user);
  const roles = runtimeRoles(state, user);
...
function hasValidToken() { return Boolean(runtimeToken()); }
function getRefreshToken() { return ""; }
function getCurrentUser() { return cloneRuntimeUser(); }
function getUser() { return getCurrentUser(); }
function getProfile() { return getCurrentUser(); }
function getCurrentSession() { return cloneRuntimeSession(); }
function getSession() { return getCurrentSession(); }
function getUserSlug() { return runtimeUserSlug(); }
function buildUserHomePathFromSlug(slug = "") { return buildUserHomePath(slug); }
function getDefaultHome() { return runtimeHomePath(); }
function getPostLoginTarget() { return isAuthenticated() ? runtimeHomePath() : ROOT_PATH; }
function getRole() { return runtimeRole() || null; }
function getRoles() { return [...runtimeRoles()]; }

function isAuthenticated() {
  const state = coreState();
  return Boolean(runtimeToken(state) && runtimeUser(state));
}

function isAdmin() {
  const state = coreState();
  return Boolean(runtimeToken(state) && runtimeUser(state) && runtimeRole(state) === "admin");
}

function hasRole(role = "") {
  const required = AppCore.normalizeRole(role);
  if (!required) return false;
  const state = coreState();
  const user = runtimeUser(state);
  if (!runtimeToken(state) || !user) return false;
  const current = runtimeRole(state, user);
...
function getPermissions() { return [...runtimePermissions()]; }
...
function hasRefreshToken() { return runtimeHasRefreshToken(); }
...
function looksLikeUser(value = null) {
  ...
  const hasEmbeddedUser = Boolean(isObject(value.user) || isObject(value.currentUser) || isObject(value.usuario) || isObject(value.me) || isObject(value.account));
  if (hasEmbeddedUser) return false;
  ...
  const hasAccountSignals = Boolean(value.active !== undefined || value.enabled !== undefined || value.disabled !== undefined || value.status !== undefined || value.estado !== undefined || value.permissions !== undefined || value.permisos !== undefined || value.clienteId !== undefined || value.tenantId !== undefined);
...
function extractToken(payload = {}) {
  return cleanToken(pick(payload, ["token", "accessToken", "access_token"]) || "");
}

function extractUser(payload = {}) {
  if (!isObject(payload)) return null;
  const explicitUser = pick(payload, ["user", "currentUser", "usuario", "me", "account"]);
  if (looksLikeUser(explicitUser)) return explicitUser;
  const profileUser = pick(payload, ["profile"]);
  if (looksLikeUser(profileUser)) return profileUser;
...
function normalizeAuthPayload(payload = {}, options = {}) {
  ...
  const current = options.allowCurrentToken === true || options.allowCurrentUser === true ? readAuthContext() : null;
  const token = extractToken(source) || (options.allowCurrentToken === true ? current?.token || "" : "");
  const user = normalizeUser(extractUser(source) || (options.allowCurrentUser === true ? current?.user || null : null));
...
function applySession(payload = {}, options = {}) {
  ...
  const hasAuthData = Boolean(normalized.token || normalized.user || normalized.session || normalized.hasRefreshToken);
...
function getPublicAuthResult(payload = {}) {
  ...
  return {
    ...
    user: context.authenticated ? publicUser(context.user) : null,
    currentUser: context.authenticated ? publicUser(context.user) : null,
    session,
    sessionData: session,
...
function getAuthModuleSnapshot() {
  ...
  return Object.freeze({
    ...
    ...result,
    isAdmin: result.authenticated && result.role === "admin",
    routes: AUTH_ROUTES,
    home: AUTH_HOME,
    ...
    session: Object.freeze({
      ...
      activeFlows: sessionState.activeFlows,
    ...
    policy: Object.freeze({
      runtimeStateZeroCopyRead: true,
      runtimeStateSingleWrite: true,
      publicUserIsolation: true,
      publicSessionIsolation: true,
...
async function login(credentials = {}, options = {}) {
  ...
  const flow = createFlowAbort(options.signal || null);
  ...
  sessionState.loginPromise = (async () => {
    try {
      ...
      if (!result.authenticated && result.hasToken) {
        try {
          result = await fetchMe({ ...options, noAutoRefresh: true, source: "Auth.login.me" });
        } catch (error) {
          if (isHttpAuthError(error)) clearSession();
          throw error;
        }
      }
...
async function refreshSession(options = {}) {
  ...
  sessionState.refreshPromise = (async () => {
    try {
      ...
      if (!result.authenticated && result.hasToken) {
        try {
          result = await fetchMe({ ...options, noAutoRefresh: true, source: "Auth.refreshSession.me" });
        } catch (error) {
          if (isHttpAuthError(error)) clearSession();
          throw error;
        }
...
async function restoreSession(options = {}) {
  ...
  sessionState.restorePromise = (async () => {
    try {
      ...
      if (hasValidToken()) {
        try {
          return await fetchMe({ ...options, noAutoRefresh: true, source: "Auth.restoreSession.me" });
        } catch (error) {
...
function validateActivationToken(payload = {}) {
  ...
  return Promise.resolve({ ok: Boolean(token), valid: Boolean(token) });
}
...
function init() {
  installHttp();
  try {
    if (isFunction(AppCore?.registerModule)) AppCore.registerModule("auth", Auth, { overwrite: true });
    else AppCore.Auth = Auth;
  } catch {}
  return Auth;
}
