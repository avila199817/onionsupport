...
export function normalizeRouteInput(path = "/") {
...
export function extractUsernameFromPath(AppCore, pathname = "/") {
  const normalized = normalizePath(AppCore, pathname);
  const pathOnly = stripSearchAndHash(normalized);
  const match = pathOnly.match(/^\/@([^/]+)(?:\/|$)/i);

  if (!match) return null;

  return sanitizeUsername(AppCore, match[1]);
}

export function stripUsernamePrefix(AppCore, pathname = "/") {
  const normalized = normalizePath(AppCore, pathname);
  ...
  return normalizePath(AppCore, `${stripped}${suffix}`);
}
...
export function normalizeCanonicalPath(AppCore, path = "/") {
  ...
  return normalizePath(AppCore, stripSearchAndHash(stripped));
}
...
export function getCurrentResolvedUsername(AppCore) {
  return (
    extractUsernameFromPath(AppCore, window.location.pathname || "/") ||
...
export function resolveSpaHref(AppCore, href = "/") {
  ...
  if (!raw) return routeNames.HOME;
  if (isUnsafeHref(raw)) return routeNames.HOME;
  ...
  if (raw.startsWith("./")) {
    return normalizePath(AppCore, `/${raw.slice(2)}`);
  }

  if (raw.startsWith("../")) {
    const cleaned = raw.replace(/^(\.\.\/)+/, "");
    return normalizePath(AppCore, `/${cleaned}`);
  }

  return normalizePath(AppCore, `/${raw}`);
}

export function buildPublicPath(AppCore, getRoute, canonicalPath = "/", options = {}) {
  const routeNames = getRouteNames(AppCore);
  const canonical = normalizeCanonicalPath(AppCore, canonicalPath);
  const route = getRoute(canonical);

  if (!route) return canonical;
  if (!canUsePublicSlugForRoute(route, routeNames)) return canonical;

  const username = sanitizeUsername(
    AppCore,
    options.username ||
      extractUsernameFromPath(AppCore, options.fromPath || "") ||
...
export function getRedirectPath(AppCore) {
  ...
  if (!canonical || canonical === routeNames.LOGIN) {
    return null;
  }
...
export function buildLoginUrl(AppCore, redirectPath = null) {
  const routeNames = getRouteNames(AppCore);
  const loginPath = normalizePath(AppCore, routeNames.LOGIN);

  if (
    !redirectPath ||
    normalizeCanonicalPath(AppCore, redirectPath) === routeNames.LOGIN
  ) {
    return loginPath;
  }
  ...
  url.searchParams.set(
    "redirect",
    normalizeCanonicalPath(AppCore, redirectPath)
  );
...
export function buildHistoryUrl(
  ...
  const routeNames = getRouteNames(AppCore);
  const normalized = resolveSpaHref(AppCore, pathname);
  const canonical = normalizeCanonicalPath(AppCore, normalized);
  const route = getRoute(canonical);
  ...
  if (!options.preservePath) {
    if (route && canUsePublicSlugForRoute(route, routeNames)) {
      finalPath = buildPublicPath(AppCore, getRoute, canonical, {
  ...
  if (options.withRedirect) {
    ...
    if (redirectCanonical && redirectCanonical !== routeNames.LOGIN) {
...
export function buildStatePayload(AppCore, pathname = "/", extras = {}) {
  const normalized = normalizePath(AppCore, pathname);
  const canonicalPath = normalizeCanonicalPath(AppCore, normalized);
  const username = extractUsernameFromPath(AppCore, normalized) || null;
...
export function getDefaultHomeTarget(AppCore, getRoute) {
  ...
  return (
    buildPublicPath(AppCore, getRoute, routeNames.HOME, {
      username:
        getCurrentResolvedUsername(AppCore) || getCurrentUsername(AppCore),
    }) || routeNames.HOME
  );
}
