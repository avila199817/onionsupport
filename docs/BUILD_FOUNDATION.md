...
immutable revision and deploys `/` without a build. The autogenerated Azure
origin must then redirect in one exact permanent hop to `https://onionsupport.com`
while preserving path and query; byte equality is checked at that canonical
serving origin, followed by the security, routing, CORS and backend verifier.
