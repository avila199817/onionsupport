...
- **El gate que valida con el tooling de la base obligó a una entrega previa.** `Validate dist with trusted base tooling` y los dos `Compare home` construyen el candidato pero ejecutan el `tools/` de `main`, a propósito: así un PR no puede relajar su propio control. Tres contratos de la base rechazaban Agenda, y ninguno por un defecto suyo: el techo del cierre bootstrap/Home, y los mapas de consumidores de `error-extraction-contract` y `format-contract`, que comparaban con `deepEqual` en los dos sentidos e impedían que **cualquier** PR estrenase un consumidor de `errorMessage` o de `dateFormatter` —empujando justo a lo contrario de lo que esos contratos defienden: duplicar la autoridad dentro de cada dominio nuevo.
