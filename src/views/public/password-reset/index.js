/* =========================================================
   ...
   Responsabilidad:
   - Controlador público de recuperación/restablecimiento de contraseña.
   - Montar exclusivamente el DOM recibido desde ./template.js.
   - Detectar modo request/reset según ruta y token.
   - Leer token desde ?token= (contrato productivo) y aliases compatibles.
   - Mantener compatibilidad defensiva con enlaces legacy por pathname.
   - Validar formulario antes de llamar Auth.
   - Llamar Auth.requestPasswordReset() o Auth.confirmResetPassword().
   - Mostrar loading/error/success.
   - Tras cambio correcto de contraseña, navegar a /login mediante Router.
   - Respetar redirectTo/redirect_to/data.redirectTo devuelto por backend.
   - No depender de sesión autenticada para completar un password reset.
   - No persistir ni loggear el token.
...
const PASSWORD_REQUEST_ROUTE =
  ROUTES.passwordRequest || "/password-request";

const PASSWORD_RESET_ROUTE =
...
const LEGACY_RESET_PREFIXES = Object.freeze([
...
function resolveResetSuccessTarget(result = {}) {
...
function pathFromContext(context = {}) {
  /*
    publicPath debe ganar a canonicalPath porque conserva ?token=.
    canonicalPath normalmente contiene sólo "/password-reset".
  */
  return cleanText(
    context.publicPath ||
      context.path ||
...
function tokenFromLegacyPathname(
  pathname = ""
) {
  ...
  for (const prefix of LEGACY_RESET_PREFIXES) {
    if (
      !path
        .toLowerCase()
        .startsWith(
...
function tokenFromPath(value = "") {
  ...
  try {
    ...
    /*
      ...
      los nuevos correos NO deben usar este formato.
    */
    return tokenFromLegacyPathname(
      url.pathname
    );
  } catch {
    return "";
...
function resolveTemplate(context = {}) {
  if (
    ...
    throw new Error(
      "[PasswordResetView] template.js debe exportar createPasswordResetTemplate()."
    );
  }
...
function getRefs(
  ...
  if (!root) {
    throw new Error(
      "[PasswordResetView] root inválido."
    );
  }
  ...
  const refs = {
    ...
    identifier:
      root.querySelector(
        "[data-password-reset-identifier]"
      ) ||
      root.querySelector(
        "[data-reset-password-identifier]"
      ) ||
      root.querySelector(
        "[name='identifier']"
      ) ||
    ...
    identifierError:
      ...
      root.querySelector(
        "[data-password-reset-error='identifier']"
      ),
    ...
    passwordError:
      ...
      root.querySelector(
        "[data-password-reset-error='password']"
      ),
    ...
    confirmPasswordError:
      ...
      root.querySelector(
        "[data-password-reset-error='confirm-password']"
      ),
  ...
  if (!refs.form) {
    throw new Error(
      "[PasswordResetView] falta [data-password-reset-form]."
    );
  }

  if (!refs.submit) {
    throw new Error(
      "[PasswordResetView] falta submit."
    );
  }
  ...
  if (
    ...
    throw new Error(
      "[PasswordResetView] falta input identifier."
    );
  }
  ...
  if (
    ...
    throw new Error(
      "[PasswordResetView] falta input password."
    );
  }
  ...
  if (
    ...
    throw new Error(
      "[PasswordResetView] falta input confirmPassword."
    );
  }
...
function getPasswordControlRefs(
  wrapper = null
) {
...
function setPasswordVisible(
  ...
  refs.input.dataset.passwordVisible =
    value
      ? "true"
      : "false";
  ...
  if (refs.wrapper) {
    refs.wrapper.dataset.passwordVisible =
      value
        ? "true"
        : "false";
  }
...
function initSinglePasswordControl(
  wrapper = null
) {
  const refs =
    getPasswordControlRefs(
      wrapper
    );
  ...
  return {
    ...
    destroy() {
      ...
      refs.toggle?.removeEventListener(
        "mousedown",
        keepFocus
      );
...
function initPasswordControls(
  ...
  const controls =
    (
      refs.passwordWrappers ||
      []
    )
      .map(
        initSinglePasswordControl
      )
      .filter(Boolean);
...
function fieldError(
  ...
  return (
    refs.root
      ?.querySelector?.(
        `[data-password-reset-error="${name}"]`
      ) ||
    null
  );
}
...
function setMessage(
  ...
  refs.message.dataset.messageType =
    cleanText(
      type,
      "info"
    );

  refs.message.classList.toggle(
    "is-error",
    type === "error"
  );
...
function setLoading(
  ...
  if (refs.submit) {
    ...
    refs.submit.dataset.loading =
      value
        ? "true"
        : "false";

    refs.submit.setAttribute(
      "aria-busy",
      value
        ? "true"
        : "false"
...
export function renderPasswordResetView(
  container,
  context = {}
) {
  if (!isBrowser()) {
  ...
  if (!container) {
    throw new Error(
      "[PasswordResetView] container requerido."
    );
  }
  ...
  const mode =
    mountedTemplate.mode;

  /*
    El token sólo vive en closure.
    No se expone en snapshot, dataset ni estado global.
  */
  const token =
  ...
  async function submit(
    ...
    try {
      if (
        ...
        /*
          Si el Router no está disponible no hacemos location.href
          paralelo: dejamos el estado success visible. De este modo se
          conserva la arquitectura SPA y no se introducen dos fuentes de
  ...
  const instance = {
    ...
    getSnapshot() {
      return {
        ...
        /*
          Sólo exponemos presencia, nunca valor.
        */
        tokenPresent:
          Boolean(token),
...
export const PasswordResetView =
  Object.assign(
    function PasswordResetViewCompat(
      container,
      context = {}
    ) {
      return renderPasswordResetView(
...
export {
  renderPasswordResetView as render,
};
