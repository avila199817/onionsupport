...
| Guards de forma: objeto plano (`isObject`, `safeObject`) | `src/core/objects.js` | Una sola definición: kernel (`core`, `http`, `app`, `router`, `auth`), UI compartida, features y vistas la importan (la base de plantillas de Home la reexporta en vivo). Un `fallback` por parámetro sustituye a las copias con `{}` o `null` por defecto. `main.js` conserva su `isObject` porque la entrada no puede importar módulos compartidos sin que rolldown los pliegue en su chunk (medido: `auth` de 63835 a 102979 bytes). `http.isPlainObject` la compone con la comprobación de prototipo. Contrato: `tools/shape-guards-contract.mjs`. |
