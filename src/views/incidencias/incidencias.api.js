...
export async function fetchIncidenciasRequest() {
  return getApiClient().get(INCIDENCIAS_ENDPOINT, {
...
export async function getIncidenciaByIdRequest(id) {
  return getApiClient().get(getTicketEndpoint(id), {
    timeout: INCIDENCIAS_TIMEOUT,
    auth: true,
  });
}
...
export async function loadIncidencias({ force = false } = {}) {
  const firstLoad = !incidenciasState?.hydrated;
  ...
  try {
    ...
    setItems(list);
    setRemoteCount(pickTotal(response, list.length));
