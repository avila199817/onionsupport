...
| Shell modal: estructura DOM, ARIA, tamaños, cierre, estados y CSS estructural | `src/features/entity-overlay/modal-shell.js` y `src/css/components/detail-modal.css` | Un solo shell (`ui-modal-shell.v1`) para todo diálogo privado; los dominios aportan contenido, acciones e identidad `data-*`. Contrato: `tools/modal-shell-contract.mjs`. |
