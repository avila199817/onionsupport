...
export const App = (() => {
  ...
  function syncLangState() {
    try {
      ...
      AppCore.setState({ lang: fallbackLang });
  ...
  function setShellVisibility(visible = true) {
    ...
    const { sidebar, topbar, tablehead, tableheadContainer, body } =
      getShellElements();
    ...
    AppCore.events.emit("router:shell:change", { hidden });
  }
  ...
  function markStoreBootState({
    ...
    if (Store?.actions?.markBooted) {
      Store.actions.markBooted(Boolean(isBooted));
    }
  }
  ...
  function initUISystems() {
    ...
    registerModule("sidebar", SidebarUI);
    registerModule("topbar", TopbarUI);
