/* =========================================================
   ...
   PRODUCTIVO · TEMPLATE PURO · FACTURAS/CLIENTES PARITY · V22

   Responsabilidad:
   - Emitir exclusivamente HTML/presentación de /usuarios.
   - Mantener seis columnas reales y fila completa interactiva.
   - Mantener filtros, búsqueda, skeleton inicial y load-more.
   - No renderizar botón ni overlay manual de actualización.
...
export const USUARIOS_TEMPLATE_VERSION =
  "usuarios.template.productivo.v22.facturas-clientes-parity";
...
export const USUARIOS_ACTIONS = Object.freeze({
  DETAIL: "detail",
  CREATE: "create",
  REFRESH: "refresh", // compat pública; no se renderiza en UI
  RETRY: "retry",
  EXPORT: "export",
  FILTER: "filter",
...
function truncate(value = "", max = 96) {
  ...
  return !text || text.length <= limit
    ? text
    : `${text.slice(0, Math.max(0, limit - 1)).trim()}…`;
}
...
function safeAvatarUrl(value = "") {
  ...
  try {
    ...
    if (queryKeys.some(isSensitiveQueryParam)) return "";

    const hasSas = queryKeys.some((key) =>
      [
        "sig",
        "se",
        "sp",
...
function formatRelativeDate(value = null) {
  ...
  if (absMinutes < 60) return diffMinutes > 0 ? `En ${absMinutes} min` : `Hace ${absMinutes} min`;
...
function getUsuarioCode(item = {}) {
  return safeText(first(item.code, item.username, getUsuarioId(item), item.email, "USR-SIN-ID"), "USR-SIN-ID");
}
...
function getUsuarioEmail(item = {}) {
  return safeText(first(item.email, item.emailLower, item.mail, ""), "").toLowerCase() || "Sin email";
}

function getUsuarioLocation(item = {}) {
  return (
    safeText(first(item.city, item.ciudad, item.direccion?.ciudad, item.address?.ciudad, ""), "") ||
    "Sin ciudad"
  );
}
...
const isBlockedLike = (item = {}) => ["blocked", "inactive"].includes(getStatusValue(item));
...
function getSearchQuery(input = {}) {
  ...
  const state = safeObject(data.state);
  return safeText(
    first(data.search, data.searchQuery, state.search, state.searchQuery, ""),
    ""
  );
}
...
function filterUsuariosForView(items = [], input = {}) {
  ...
  return safeArray(items).filter(
    (item) => itemMatchesFilter(item, filter) && itemMatchesSearch(item, search)
  );
}
...
function getFilterLabel(filter = "all") {
  return FILTERS.find((item) => item.key === normalizeFilter(filter))?.label || "Todos";
}
  ...
  return `
    ...
    >
      ...
      <td class="usuarios-cell usuarios-cell--date" data-column="date">
        <span class="usuarios-date-inline" title="${attr(formatDateTime(createdRaw))}">${escapeHtml(formatDateShort(createdRaw))}</span>
      </td>
      <td class="usuarios-cell usuarios-cell--email" data-column="email">
        <span class="usuarios-email-inline" title="${attr(getUsuarioEmail(item))}">${escapeHtml(getUsuarioEmail(item))}</span>
      </td>
      <td class="usuarios-cell usuarios-cell--location" data-column="location">
        <span class="usuarios-location-inline" title="${attr(getUsuarioLocation(item))}">${escapeHtml(getUsuarioLocation(item))}</span>
      </td>
...
function renderFilters(input = {}, pagination = {}) {
  ...
  const active = normalizeFilter(pagination.activeFilter || getActiveFilter(data));
...
export function renderHeader(input = {}) {
  ...
  const title = safeText(first(data.title, state.title, "Usuarios"), "Usuarios");
  const subtitle = safeText(
    first(
      data.subtitle,
      state.subtitle,
      "Gestiona usuarios, estados, accesos y contactos desde un único historial."
    ),
    ""
  );
  ...
  const busy = creating || loading || exporting;
    ...
    >
            ...
            ? `<div class="usuarios-hero-actions">
                <button
                  ...
                  ${htmlAttrs({
                    ...
                    "aria-disabled": loading || exporting || !items.length ? "true" : false,
      ...
      <div class="usuarios-stats">
        <article class="usuarios-stat-card usuarios-stat-card--total" data-stat="total">
          <div class="usuarios-stat-label">Usuarios</div>
          <div class="usuarios-stat-value">${escapeHtml(formatNumber(stats.total))}</div>
          <div class="usuarios-stat-text">Cuentas cargadas en el historial actual.</div>
        </article>
...
function renderEmptyContent({
  ...
  const text = hasError
    ? safeText(message, "Puedes reintentar la carga.")
    : filtering
      ? searchQuery
      ...
      ${
        hasError
          ? `<button
              ...
              data-action="retry"
            >${icon("refresh")}<span class="usuarios-btn-text">Reintentar</span></button>`
          : filtering
            ? `<button
                type="button"
                class="usuarios-btn"
              ...
              >${icon("close")}<span class="usuarios-btn-text">Limpiar filtros</span></button>`
            : allowCreate
              ? `<button
                  type="button"
                  ...
                  data-usuarios-action="${USUARIOS_ACTIONS.CREATE}"
                  data-action="create-user"
                >${icon("plus")}<span class="usuarios-btn-text">Crear usuario</span></button>`
              : ""
      }
    </div>
  `;
...
function renderFeedFooter(pagination = {}, state = {}) {
  const runtime = safeObject(state);
  const disabled = Boolean(runtime.loading);
...
export function renderTable(input = {}) {
  ...
  return `
    <section
      class="usuarios-history${loading ? " is-loading" : ""}${hasError ? " has-error" : ""}"
    ...
    >
      ...
      ${
        initialLoading
          ? renderTableLoading(pagination.visibleLimit)
          : `<div class="usuarios-table-wrap" data-usuarios-table-wrap="true">
              ${
                pagination.pageItems.length
                  ? `<div class="usuarios-table-shell">
                      ...
                      >
                        ${renderColgroup()}
                        ${renderThead()}
                        <tbody>${pagination.pageItems.map((item) => renderRow(item, state)).join("")}</tbody>
                      </table>
                    </div>
...
export function renderUsuariosTableTemplate(input = {}) {
  ...
  return `
    <section
      class="usuarios-view-root${loading ? " is-loading" : ""}${error ? " has-error" : ""}"
      ${rootAttrs}
      aria-busy="${loading ? "true" : "false"}"
    >
