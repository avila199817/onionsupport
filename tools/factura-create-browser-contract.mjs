...
try {
  ...
  await check('primary-client tax state, multiline draft and stable line identity survive changes',async()=>{
    ...
    await p.locator('[data-line-field="descripcion"]').fill('Primera linea\nSegunda linea');
    await p.locator(action('line-add')).click();
    ...
    await choose(p,'Nuevo');await p.locator(action('client-primary')).click();
    ...
    assert.equal(await p.locator('[data-line-field="descripcion"]').first().inputValue(),'Primera linea\nSegunda linea');
    await p.locator(action('line-remove')).first().click();
  ...
  await check('validation and duplicate submit keep a single modal and immutable selected payload',async()=>{
    ...
    await p.locator(action('submit')).click();assert.match(await p.locator('[data-error-slot="clienteId"]').innerText(),/Selecciona/);
    ...
    await p.locator(action('submit')).click();await p.waitForFunction(()=>creates.length===1);
  ...
  await check('close/reopen and destroy reject late results; keyboard close restores focus',async()=>{
    ...
    assert.equal(await p.evaluate(()=>document.activeElement.id),'opener');
