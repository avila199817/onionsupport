/* =========================================================
   Onion SPA - Clientes Modal
   Archivo: src/views/clientes/clientes.modal.js

   FINAL PRO SYSTEM · DETAIL MODAL · CLIENTES EDITION

   RESPONSABILIDADES:
   - renderizar modal premium de detalle de cliente
   - abrir / cerrar modal limpio
   - refrescar contenido del cliente desde modal
   - copiar ID cliente
   - soportar timeline / docs / metadata
   - bridge global para clientesView.js
...
export function openClientesModal(
  detail = {}
) {
  modalState.detail =
    getDetail(detail);
...
export function closeClientesModal() {
  const root = getRoot();
...
export const OnionClientesModal =
{
  open: openClientesModal,
  close: closeClientesModal,
};

try {
  window.OnionClientesModal =
    OnionClientesModal;

  window.renderClienteModal =
    OnionClientesModal.open;
} catch {}
...
export default OnionClientesModal;
