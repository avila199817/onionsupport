...
| Alta de Agenda («Crear cita») | shell canónico (`renderModalShell`, variante `data-modal-size="form"`, altura `auto`), botón cerrar del shell, acción principal en el slot `footer` con `form=`, click exterior por el lifecycle; reutiliza la composición de altas (`inc-create-*`) y el combobox accesible de selección de usuario mediante el marcador compartido `data-create-user-picker-root`; `views/agenda/index.css` estiliza sólo el contenido |
| Detalle de Agenda (cita) | shell canónico (`renderModalShell`, tamaño `compact`, altura `auto`), botón cerrar del shell, estados de carga y error compartidos, click exterior por el lifecycle; la confirmación de salida del alta usa `openModalConfirmation` |
...
| Confirmaciones asíncronas | `src/features/entity-overlay/modal-confirmation.js` (`openModalConfirmation`) | `facturas-confirmation-browser-contract.mjs` (11 escenarios) | Reenvío y cobro de Facturas, salida con borrador del alta de Agenda |
| Selección de usuario en un alta (teclado, ARIA, `aria-activedescendant`, IME, Enter que no envía el formulario) | `src/views/incidencias/incidencias.create-user-combobox.js` (`incidencias.create-user-combobox.v3-shared-root`) | `.github/scripts/incidencias_create_combobox_contract.mjs`, `tools/agenda-citas-browser-contract.mjs` | Toda alta que declare `data-create-user-picker-root` y el contrato de campos (`data-create-user-search-input`, `data-create-user-results`, `role="option"` + `data-create-action="create-user-select"`): Incidencias y Agenda |
