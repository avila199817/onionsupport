/* =========================================================
   ...
   - restaurar sesión con token o solo con refresh context
   - evitar carreras de me / refresh / restore
...
export async function fetchMe(session) {
  ...
  session.mePromise = (async () => {
    try {
      const response = await AppCore.apiClient.get(
        AUTH_ENDPOINTS.me,
        { auth: true }
      );
...
export async function restoreAfterMeFailure(session, meError) {
  AppCore.utils.warn(
...
export async function restoreSession(session) {
  ...
  session.restorePromise = (async () => {
    try {
      ...
      try {
        return await restoreUsingMe(session);
      } catch (meError) {
        return await restoreAfterMeFailure(session, meError);
      }
    } finally {
