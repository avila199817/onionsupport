/* =========================================================
   ...
   Local thumbnails + signed thumbnail recovery · V1

   UI-only progressive feature. It never mutates ticket state, File objects,
   payloads, permissions or HTTP contracts. Remote previews always use the
   existing incidencias.api.js /view contract.
========================================================= */

export const INCIDENCIAS_MEDIA_PREVIEW_VERSION = "incidencias-media-preview.extreme.v1";
...
const array = (value) => { try { return value ? Array.from(value) : []; } catch { return []; } };
const fileLike = (file) => Boolean(file && typeof file === "object" && typeof file.name === "string" && typeof file.size === "number" && (typeof file.arrayBuffer === "function" || typeof file.stream === "function" || typeof file.slice === "function"));
const signature = (file = {}) => [text(file.name, "archivo"), Number(file.size || 0), Number(file.lastModified || 0), text(file.type).toLowerCase()].join("::");
const imageFile = (file = {}) => text(file.type).toLowerCase().startsWith("image/") || IMAGE_RE.test(text(file.name));
const labelFor = (file = {}) => {
...
function registerFiles(values) {
  ...
  if (!files.length || files.some((file) => Number(file.size || 0) > MAX_SIZE)) return false;
  ...
  for (const { key, file } of incoming) {
    pending.entries.set(key, { key, file, image: imageFile(file), label: labelFor(file), url: objectUrl(file) });
...
function imageState(img, state) {
  const box = img?.closest?.("[data-local-preview-frame='true'], [data-modal-thumb-frame='true']");
  if (!box) return false;
...
function bindImage(img, remote = false) {
  ...
  const loaded = () => imageState(img, img.naturalWidth > 0 && img.naturalHeight > 0 ? "ready" : "error");
  const failed = () => {
...
function syncPending(root) {
  const rows = [...root.querySelectorAll(".incidencias-modal-pending-file[data-file-index]")];
  if (!rows.length) {
...
function syncSlots(root) {
  ...
  if (feedback) feedback.dataset.slotEmpty = feedback.querySelector(".incidencias-modal-feedback") ? "false" : "true";
  const preview = root.querySelector("[data-modal-preview-slot='true']");
  if (preview) preview.dataset.slotEmpty = preview.querySelector(".incidencias-modal-preview") ? "false" : "true";
}

const remoteKey = (ticketId, attachmentId) => text(ticketId) && text(attachmentId) ? `${text(ticketId)}::${text(attachmentId)}` : "";
...
function expiry(url) {
  ...
  } catch { return Date.now() + DEFAULT_TTL; }
}
...
const viewUrl = (file = {}) => text(file.viewUrl || file.openUrl || file.signedUrl || file.sasUrl || file.url);
...
async function requestUrl(ticketId, attachmentId, force = false) {
  ...
  const task = (async () => {
    try {
      const file = await (await api()).openIncidenciaAttachment({ ticketId, attachmentId });
...
function decorate(frame, img) {
  if (!frame || !img) return;
  if (boundImages.has(img)) {
    if (img.complete && img.naturalWidth > 0 && img.naturalHeight > 0) imageState(img, "ready");
    return;
  }
  frame.dataset.previewState = "loading";
  frame.dataset.thumbError = "false";
  bindImage(img, true);
}
...
function installRemoteThumb(button, url) {
  ...
  const name = text(button.getAttribute("aria-label"), "Imagen adjunta").replace(/^Ver\s+/i, "").replace(/^Ampliar\s+/i, "");

  button.classList.remove("incidencias-modal-file-square");
  button.classList.add("incidencias-modal-image-thumb-wrap");
  Object.assign(button.dataset, { renderableThumbnail: "true", modalThumbFrame: "true", previewState: "loading", thumbError: "false", attachmentId: id });
...
async function hydrateFallback(button) {
  ...
  if (text(button.querySelector(":scope > span")?.textContent).toUpperCase() !== "IMG") return;
...
function syncRemote(root) {
  ...
  for (const button of root.querySelectorAll(".incidencias-modal-file-square[data-renderable-thumbnail='false']")) void hydrateFallback(button);
}
...
function sync() {
  ...
  if (!root) {
    if (lastRoot) { clearPending(); clearRemote(); }
    lastRoot = null;
    return;
  }
  if (lastRoot && lastRoot !== root) { clearPending(); clearRemote(); }
  lastRoot = root;
  syncSlots(root);
  syncPending(root);
  syncRemote(root);
...
function schedule() {
  if (!browser() || destroyed || frame) return;
...
function onClick(event) {
  ...
  if (!button) return;
  removePending(Number(button.dataset.fileIndex ?? button.dataset.removeAttachment ?? -1));
  schedule();
}
...
export default mountIncidenciasMediaPreview;
