...
| Normalización de texto en una línea (`cleanText`) | `src/core/presentation-text.js` | Una sola definición: kernel (`core`, `http`, `app`, `loader`, `router`), `auth`, vistas públicas, features y vistas privadas la importan (o la reexportan en vivo, como Home y la vista pendiente); las variantes de dominio la componen (`clipText` en topbar-executive, `cleanMessageText` en WhatsApp). El módulo sólo contiene el normalizador para que los cierres de arranque no carguen más. Contrato: `tools/presentation-text-contract.mjs`. |
