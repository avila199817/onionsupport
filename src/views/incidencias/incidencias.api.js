/* =========================================================
   ...
   API única de Incidencias:
   - /api/tickets como endpoint canónico.
   - Normalización frontend/backend 1:1.
   - Create payload sin inventar clienteId.
   - Compatible con targetUserId/targetClienteId/email/avatar.
   - Cache en memoria, dedupe de listados y stale-on-error.
   - Sin DOM, sin Router, sin Store, sin fetch propio.
========================================================= */

import Http from "../../core/http.js";

export const INCIDENCIAS_API_VERSION = "incidencias.api.cached.v4.backend-1to1";
...
function safePublicText(value = "", fallback = "") {
  ...
  if (/[?&#](?:token|access_token|refresh_token|password|secret|sig|signature)=/i.test(text)) return fallback;
  return text;
}
...
function getIncidenciaStableId(item = {}) {
  return cleanText(first(item.ticketId, item.incidenciaId, item.id, item._id, item.code, item.numero, item.ticketCode), "");
}
...
function mergeIncidenciaData(current = {}, next = {}) {
  const output = { ...safeObject(current, {}) };
  ...
  for (const [key, value] of Object.entries(incoming)) {
    ...
    output[key] = shouldPreserveExisting(value) && previous !== undefined && previous !== null ? previous : value;
  }
...
function incidenciaSortTime(item = {}) {
  const timestamp = Date.parse(first(item.lastActivityAt, item.updatedAt, item.modifiedAt, item.closedAt, item.createdAt, item.lifecycle?.lastActivityAt, item.lifecycle?.updatedAt, item.lifecycle?.closedAt, item.lifecycle?.createdAt, 0));
...
function upsertCachedIncidencia(item = null) {
  ...
  const map = new Map();
  const existing = safeArray(lastList.items).find((current) => getIncidenciaStableId(current) === id) || null;
  map.set(id, existing ? mergeIncidenciaData(existing, normalized) : normalized);
  ...
  lastList = { items, total: Math.max(number(lastList.total, items.length), items.length) };
...
export function normalizeIncidenciaId(id = "") {
  ...
  if (!value) throw new Error("INCIDENCIA_ID_REQUIRED");
  return value;
}
...
export function getIncidenciaAttachmentFileEndpoint({ ticketId = "", attachmentId = "", mode = "view", kind = "attachments" } = {}) {
  ...
  const safeKind = ["attachments", "files", "adjuntos"].includes(kind) ? kind : "attachments";
...
function unwrapEnvelope(payload = null, depth = 0) {
  ...
  if (["items", "rows", "results", "records", "docs", "documents", "value", "list", "tickets", "incidencias"].some((key) => Array.isArray(object[key]))) return object;
  if (object.ticket || object.incidencia || object.item || object.detail || object.file || object.attachment || object.adjunto) return object;

  const nested = first(object.data, object.payload, object.result, object.response, object.body);
  return nested !== null && nested !== undefined && nested !== payload ? unwrapEnvelope(nested, depth + 1) : object;
}

function listFromPayload(payload = null) {
  if (Array.isArray(payload)) return payload;
  const object = safeObject(unwrapEnvelope(payload), {});
  for (const key of ["items", "rows", "records", "results", "docs", "documents", "value", "list", "tickets", "incidencias"]) {
...
function totalFromPayload(payload = null, fallback = 0) {
  ...
  return Math.max(fallback, number(first(envelope.total, envelope.count, envelope.totalCount, envelope.remoteCount, envelope.meta?.total, envelope.meta?.count, envelope.pagination?.total, envelope.pagination?.totalCount, object.total, object.count, object.totalCount, object.remoteCount, fallback), fallback));
}

function looksLikeIncidencia(value = null) {
  const item = safeObject(value, null);
  if (!item) return false;
  return Boolean(item.ticketId || item.incidenciaId || item.id || item._id || item.code || item.numero || item.ticketCode || item.subject || item.asunto || item.title || item.message || item.description || item.descripcion);
}
...
function fileFromPayload(payload = null) {
  if (isBlob(payload)) return { blob: payload, size: payload.size, contentType: payload.type };
  const object = safeObject(unwrapEnvelope(payload), {});
  return safeObject(first(object.file, object.attachment, object.adjunto, object.data?.file, object.data?.attachment, object.data?.adjunto, object.payload?.file, object.payload?.attachment, object.payload?.adjunto, object.result?.file, object.result?.attachment, object.result?.adjunto, object), {});
}
...
function normalizePerson(value = {}) {
  ...
  const email = firstEmail(raw.email, raw.emailLower, raw.userEmail, raw.mail, raw.profile?.email, raw.auth?.email, raw.lookup?.emailLower);
  const avatarUrl = firstUrl(raw.avatarUrl, raw.avatar, raw.picture, raw.photoUrl, raw.photoURL, raw.imageUrl, raw.profile?.avatarUrl, raw.profile?.avatar, raw.profile?.photoUrl, raw.profile?.photoURL);
  const name = cleanText(first(raw.displayName, raw.fullName, raw.name, raw.nombre, raw.profile?.displayName, raw.profile?.name, raw.username), "");
...
function normalizeAttachment(file = {}, index = 0) {
  ...
  const id = cleanText(first(raw.id, raw.fileId, raw.attachmentId, raw.blobName, raw.storageKey, raw.path, raw.key, nested.id, nested.fileId, nested.attachmentId, nested.blobName, nested.storageKey, nested.path, nested.key), `attachment-${index + 1}`);
  const name = cleanText(first(raw.name, raw.filename, raw.fileName, raw.title, nested.name, nested.filename, nested.fileName, nested.title), `archivo_${index + 1}`);
  const url = firstUrl(raw.viewUrl, raw.openUrl, raw.signedUrl, raw.url, raw.blobUrl, raw.publicUrl, raw.downloadUrl, nested.viewUrl, nested.openUrl, nested.signedUrl, nested.url, nested.blobUrl, nested.publicUrl, nested.downloadUrl);
...
function normalizeTimelineEntry(entry = {}, index = 0) {
  ...
  return {
    ...
    kind,
    type,
    title: cleanText(first(raw.title, kind === "comment" ? "Comentario" : type === "created" ? "Incidencia creada" : "Actualización"), "Actualización"),
    body: cleanText(first(raw.body, raw.message, raw.text, raw.comment, raw.description, raw.detail), kind === "comment" ? "" : "Actualización registrada."),
    author: cleanText(first(raw.author, raw.byName, raw.user, raw.name, raw.createdBy?.name, raw.createdBy?.displayName), kind === "comment" ? "Usuario" : "Sistema"),
...
function normalizeTimeline(item = {}) {
  ...
  if (timeline.length) return timeline.map(normalizeTimelineEntry);
...
export function normalizeIncidencia(item = {}) {
  const raw = safeObject(item);
  const ticketId = cleanText(first(raw.ticketId, raw.incidenciaId, raw.id, raw._id, raw.code, raw.numero, raw.ticketCode), "");
  ...
  const requesterName = safePublicText(first(raw.requesterName, raw.ownerName, raw.clientName, raw.clienteName, raw.clienteNombre, raw.userName, raw.usuarioName, raw.name, raw.requesterSnapshotName, raw.requesterSnapshotNombre, raw.requesterSnapshot?.displayName, raw.requesterSnapshot?.name, raw.requesterSnapshot?.nombre, raw.cliente?.displayName, raw.cliente?.name, raw.cliente?.nombre, raw.client?.displayName, raw.client?.name, raw.user?.displayName, raw.user?.name), "Usuario");
  const requesterEmail = firstEmail(raw.requesterEmail, raw.requesterEmailLower, raw.clientEmail, raw.clientEmailLower, raw.clienteEmail, raw.clienteEmailLower, raw.userEmail, raw.userEmailLower, raw.email, raw.emailLower, raw.emailCliente, raw.rootEmail, raw.rootUserEmail, raw.rootClienteEmail, raw.rootClienteEmailLower, raw.requesterSnapshotEmail, raw.requesterSnapshotEmailLower, raw.clienteEmailNested, raw.clienteEmailLowerNested, raw.receptorEmail, raw.receptorEmailLower, raw.createdByEmail, raw.createdByEmailLower, raw.requesterSnapshot?.email, raw.requesterSnapshot?.emailLower, raw.cliente?.email, raw.cliente?.emailLower, raw.client?.email, raw.client?.emailLower, raw.usuario?.email, raw.usuario?.emailLower, raw.user?.email, raw.user?.emailLower, raw.receptor?.email, raw.receptor?.emailLower, raw.createdBy?.email, raw.createdBy?.emailLower, raw.meta?.requesterEmail, raw.meta?.clientEmail, raw.meta?.clienteEmail, raw.meta?.userEmail);
  const requesterAvatarUrl = firstUrl(raw.requesterAvatarUrl, raw.requesterAvatar, raw.clientAvatarUrl, raw.clientAvatar, raw.clienteAvatarUrl, raw.clienteAvatar, raw.userAvatarUrl, raw.userAvatar, raw.avatarUrl, raw.avatar, raw.photoUrl, raw.photoURL, raw.picture, raw.requesterSnapshotAvatarUrl, raw.requesterSnapshotAvatar, raw.receptorAvatarUrl, raw.receptorAvatar, raw.createdByAvatarUrl, raw.createdByAvatar, raw.requesterSnapshot, raw.cliente, raw.client, raw.user, raw.receptor, raw.createdBy, raw.meta?.requesterAvatarUrl, raw.meta?.requesterAvatar, raw.meta?.clientAvatarUrl, raw.meta?.clientAvatar);
  const requesterUserId = cleanText(first(raw.userId, raw.usuarioId, raw.requesterUserId, raw.ownerUserId, raw.createdByUserId, raw.receptorUserId, raw.requesterSnapshotUserId, raw.clienteUserId, raw.createdByUserIdNested, raw.userRef?.userId, raw.requesterSnapshot?.userId, raw.requesterSnapshot?.id, raw.cliente?.userId, raw.client?.userId, raw.user?.userId, raw.user?.id), "");
  const requesterClienteId = cleanText(first(raw.clienteId, raw.clientId, raw.customerId, raw.targetClienteId, raw.requesterSnapshotClienteId, raw.clienteClienteId, raw.receptorClienteId, raw.clienteRef?.clienteId, raw.requesterSnapshot?.clienteId, raw.cliente?.clienteId, raw.cliente?.id, raw.client?.clienteId, raw.client?.id), "");
  const requesterUsername = cleanText(first(raw.requesterUsername, raw.username, raw.userName, raw.requesterSnapshotUsername, raw.clienteUsername, raw.receptorUsername, raw.createdByUsername, raw.requesterSnapshot?.username, raw.cliente?.username, raw.client?.username, raw.user?.username), "");

  const assignedTo = normalizePerson(first(raw.assignedTo, raw.tecnico, raw.technician, raw.agent, raw.assignedTechnician, raw.assignedUser, raw.assignment?.technician, raw.assignment?.assignedTo, {}));
  const assignedToName = safePublicText(first(raw.assignedToName, raw.technicianName, raw.tecnicoName, raw.assigneeName, raw.agentName, raw.assignmentAssignedToName, raw.assignmentTechnicianName, raw.assignmentTechnicianNombre, raw.assignmentName, raw.assignedToNombre, raw.tecnicoNombre, raw.metaTechnicianName, raw.metaAssignedTechnicianName, raw.metaLastTechnicianName, raw.assignment?.assignedToName, raw.assignment?.technicianName, raw.assignment?.agentName, raw.assignment?.name, raw.meta?.technicianName, raw.meta?.assignedTechnicianName, raw.meta?.lastTechnicianName, assignedTo.displayName, assignedTo.name), "Sin asignar");
  const assignedToEmail = firstEmail(raw.assignedToEmail, raw.technicianEmail, raw.tecnicoEmail, raw.agentEmail, raw.assignmentAssignedToEmail, raw.assignmentTechnicianEmail, raw.assignmentEmail, raw.metaTechnicianEmail, raw.metaAssignedTechnicianEmail, raw.metaLastTechnicianEmail, raw.assignment?.assignedToEmail, raw.assignment?.technicianEmail, raw.assignment?.agentEmail, raw.assignment?.email, raw.assignedTo?.email, raw.technician?.email, raw.tecnico?.email, raw.agent?.email, raw.meta?.technicianEmail, raw.meta?.assignedTechnicianEmail, raw.meta?.lastTechnicianEmail, assignedTo.email, assignedTo.emailLower);
  const assignedToAvatarUrl = firstUrl(raw.assignedToAvatarUrl, raw.assignedToAvatar, raw.technicianAvatarUrl, raw.technicianAvatar, raw.tecnicoAvatarUrl, raw.tecnicoAvatar, raw.agentAvatarUrl, raw.agentAvatar, raw.assignmentAssignedToAvatarUrl, raw.assignmentAssignedToAvatar, raw.assignmentTechnicianAvatarUrl, raw.assignmentTechnicianAvatar, raw.assignmentAvatarUrl, raw.assignmentAvatar, raw.metaTechnicianAvatarUrl, raw.metaTechnicianAvatar, raw.metaAssignedTechnicianAvatarUrl, raw.metaAssignedTechnicianAvatar, raw.metaLastTechnicianAvatarUrl, raw.metaLastTechnicianAvatar, raw.assignment?.assignedToAvatarUrl, raw.assignment?.assignedToAvatar, raw.assignment?.technicianAvatarUrl, raw.assignment?.technicianAvatar, raw.assignment?.agentAvatarUrl, raw.assignment?.agentAvatar, raw.assignment?.avatarUrl, raw.assignment?.avatar, raw.assignedTo, raw.technician, raw.tecnico, raw.agent, raw.meta?.technicianAvatarUrl, raw.meta?.technicianAvatar, raw.meta?.assignedTechnicianAvatarUrl, raw.meta?.assignedTechnicianAvatar, raw.meta?.lastTechnicianAvatarUrl, raw.meta?.lastTechnicianAvatar, assignedTo.avatarUrl, assignedTo.avatar);
  const assignedToUserId = cleanText(first(raw.assignedToUserId, raw.technicianUserId, raw.tecnicoUserId, raw.agentUserId, raw.assignmentAssignedToUserId, raw.assignmentTechnicianUserId, raw.assignmentTechnicianId, raw.assignmentUserId, raw.assignedToUserIdNested, raw.assignedToId, raw.tecnicoId, raw.metaTechnicianUserId, raw.metaAssignedTechnicianUserId, raw.metaLastTechnicianUserId, raw.assignment?.assignedToUserId, raw.assignment?.technicianUserId, raw.assignment?.userId, raw.assignedTo?.userId, raw.assignedTo?.id, raw.technician?.userId, raw.technician?.id, raw.tecnico?.userId, raw.tecnico?.id, raw.agent?.userId, raw.agent?.id, assignedTo.userId, assignedTo.id), "");
  ...
  const technician = {
    ...
    assigned: Boolean(assignedToUserId || assignedToEmail || assignedToAvatarUrl || (assignedToName && normalizeKey(assignedToName) !== "sin_asignar")),
  };
  ...
  const attachmentsCount = countFrom(attachments.length, raw.attachmentsCount, raw.filesCount, raw.adjuntosCount, raw.meta?.attachmentsCount, raw.meta?.filesCount);
  ...
  const currency = cleanText(first(raw.currency, raw.moneda, raw.facturaCurrency, raw.facturaMoneda, raw.linkedInvoicesCurrency, raw.linkedInvoicesMoneda, raw.linkedInvoices?.currency, raw.linkedInvoices?.moneda, raw.meta?.invoiceCurrency, "EUR"), "EUR").toUpperCase();
  ...
  return {
    ...
    lifecycle: { ...safeObject(raw.lifecycle), createdAt, updatedAt, lastActivityAt, closedAt },
    meta: {
      ...safeObject(raw.meta),
      requesterEmail: requesterEmail || null,
      requesterAvatar: requesterAvatarUrl || null,
...
function normalizeList(items = []) {
  ...
  return [...map.values()].sort((a, b) => {
    ...
    return getIncidenciaStableId(b).localeCompare(getIncidenciaStableId(a), "es", { numeric: true, sensitivity: "base" });
  });
}

function normalizeFileResponse(response = null, fallback = {}) {
  const file = fileFromPayload(response);
  const source = { ...safeObject(fallback), ...file };
  const url = firstUrl(source.url, source.viewUrl, source.openUrl, source.downloadUrl, source.signedUrl, source.blobUrl, source.publicUrl, source.href);
  return {
    ...source,
    url,
...
function extractFiles(payload = {}) {
  return safeArray(first(safeObject(payload).files, safeObject(payload).attachments, safeObject(payload).adjuntos, safeObject(payload).uploads, []))
    .filter((file) => isFile(file) || isBlob(file));
}

function withoutFileFields(payload = {}) {
  const output = {};
  for (const [key, value] of Object.entries(safeObject(payload))) {
...
function normalizeCreatePayload(payload = {}) {
  ...
  const targetUserId = cleanText(first(source.targetUserId, source.userId, source.usuarioId, source.user?.userId, source.user?.id, source.usuario?.userId, source.usuario?.id, ""), "");
  ...
  const targetUserName = cleanText(first(source.targetUserName, source.receptorName, source.userName, source.clienteNombre, source.clientName, source.name, source.nombre, source.user?.displayName, source.user?.name, source.cliente?.displayName, source.cliente?.name, source.cliente?.nombre, ""), "");
  const targetUserEmail = firstEmail(source.targetUserEmail, source.receptorEmail, source.userEmail, source.clienteEmail, source.clientEmail, source.email, source.emailLower, source.user?.email, source.cliente?.email, source.client?.email);
...
function buildFormData(payload = {}) {
  ...
  for (const [key, value] of Object.entries(cleanPayload)) {
    if (value === undefined || value === null) continue;
    if (typeof value === "object" && !isBlob(value) && !isFile(value)) formData.append(key, JSON.stringify(value));
...
function buildMutationBody(payload = {}) {
  const files = extractFiles(payload);
  if (files.length && typeof FormData !== "undefined") return { body: buildFormData(payload), hasFiles: true };
  return { body: normalizeCreatePayload(payload), hasFiles: false };
}

function buildAttachmentsFormData(files = [], extra = {}) {
  const formData = new FormData();
  for (const file of safeArray(files)) {
    if (isFile(file) || isBlob(file)) formData.append("attachments", file, file?.name || "archivo");
  }
...
export async function listIncidencias(options = {}) {
  ...
  inFlightListPromise = (async () => {
    try {
      ...
      return cachedListResponse({ cached: false, stale: false, options });
    } catch (error) {
...
export async function getIncidenciaByIdRequest(id = "", { timeout = INCIDENCIAS_DETAIL_TIMEOUT } = {}) {
  const response = await getJson(getIncidenciaEndpoint(id), { timeout });
...
export async function updateIncidenciaRequest(id = "", payload = {}, { timeout = INCIDENCIAS_TIMEOUT, method = "PATCH" } = {}) {
  ...
  const response = verb === "PUT"
    ...
    : await patchJson(endpoint, safeObject(payload), { timeout });
  const detail = detailFromPayload(response);
  return detail ? normalizeIncidencia(detail) : null;
}
...
export async function commentIncidenciaRequest(id = "", message = "", { timeout = INCIDENCIAS_TIMEOUT, status = "open" } = {}) {
  ...
  const response = await postJson(getIncidenciaCommentsEndpoint(id), {
    message: text,
    text,
    comment: text,
    status,
...
export async function reopenIncidenciaRequest(id = "", { timeout = INCIDENCIAS_TIMEOUT } = {}) {
  const response = await postJson(getIncidenciaReopenEndpoint(id), { status: "open", estado: "open" }, { timeout });
  const detail = detailFromPayload(response);
  return detail ? normalizeIncidencia(detail) : null;
}
...
export async function getIncidenciaAttachmentFileRequest({ ticketId = "", attachmentId = "", mode = "view", kind = "attachments" } = {}, { timeout = INCIDENCIAS_DETAIL_TIMEOUT } = {}) {
  ...
  const response = await getJson(endpoint, { timeout });
...
export async function downloadIncidenciaAttachment({ ticketId = "", attachmentId = "", kind = "attachments", filename = "" } = {}, { timeout = INCIDENCIAS_DETAIL_TIMEOUT, autoDownload = true } = {}) {
  ...
  return Http.downloadBlob(endpoint, { timeout, autoDownload, filename, source: "views.incidencias.download" });
}
...
function isOpenStatus(value = "") {
  return ["open", "pending", "in_progress", "progress", "abierta", "pendiente", "proceso"].includes(normalizeKey(value));
}

function isUrgentPriority(value = "") {
  return ["urgent", "urgente", "high", "alta", "critical", "critica", "critico"].includes(normalizeKey(value));
}
