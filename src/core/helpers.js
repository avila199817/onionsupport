/* =========================================================
   ...
   - sin estado global, sin fetch, sin Auth/Router/Store/Toast
========================================================= */
...
const FRONTEND_API_ORIGINS = Object.freeze([
...
const USER_WRAPPER_KEYS = Object.freeze([
  "user",
  "usuario",
  "me",
  "account",
...
export function getStoragePrefix() {
  return safeText(config?.storagePrefix || config?.appKey || config?.appId || DEFAULT_STORAGE_PREFIX, DEFAULT_STORAGE_PREFIX)
...
function cloneDeep(value, fallback = null, depth = 0, seen = new WeakMap()) {
  ...
  for (const [key, item] of Object.entries(value).slice(0, 300)) {
    if (typeof item !== "function") out[key] = cloneDeep(item, null, depth + 1, seen);
  }
...
export function safeClone(value, fallback = null) {
  ...
  try {
    return cloneDeep(value, fallback);
  } catch {
    return fallback;
  }
}
...
function allTokenParamNames() {
  try {
    return unique([
      ...TOKEN_PARAM_NAMES,
      ...Object.values(config?.auth?.tokenParamNames || {}).flat(),
...
export function redactTokenInText(value = "") {
  ...
  for (const name of allTokenParamNames()) {
    try {
      out = out.replace(new RegExp(`([?&#]${escapeRegExp(name)}=)([^&#\\s]+)`, "gi"), "$1***");
    } catch {}
  }

  for (const path of TECHNICAL_TOKEN_PATHS) {
    try {
      out = out.replace(new RegExp(`(${escapeRegExp(path)})\\/([^/?#\\s]+)`, "gi"), "$1/***");
    } catch {}
  }
...
export function redactSensitiveObject(value, fallback = null, depth = 0, seen = new WeakMap()) {
  ...
  for (const [key, item] of Object.entries(value).slice(0, 300)) {
    out[key] = SENSITIVE_KEY_RE.test(key) ? (item ? "***" : item) : redactSensitiveObject(item, null, depth + 1, seen);
  }
...
export function getBaseOrigin() {
  return isBrowser() && window.location?.origin ? window.location.origin : LOCAL_ORIGIN;
}
...
function originFromUrl(value = "") {
  try {
    return new URL(safeText(value, ""), getBaseOrigin()).origin;
  } catch {
    return "";
  }
}
...
export function getSearchAndHash(path = DEFAULT_ROUTE) {
  const p = splitPathParts(path);
  return `${p.search}${p.hash}`;
}
...
export function normalizeCanonicalPath(path = DEFAULT_ROUTE) {
  return collapseTechnicalTokenPath(stripSearchAndHash(stripUsernamePrefix(normalizePath(path))) || DEFAULT_ROUTE);
}
...
function canonicalBackendOrigins() {
  return unique([
    API_CANONICAL,
...
function forbiddenFrontendOrigins() {
  const backends = canonicalBackendOrigins();
  return unique([
  ...
  ]).map(originFromUrl).filter(Boolean).filter((origin) => !backends.includes(origin));
}

function isForbiddenFrontendApiBase(value = "") {
  const origin = originFromUrl(value);
  return Boolean(origin && forbiddenFrontendOrigins().includes(origin));
}
...
function basePathFromUrl(base = "") {
  ...
  try {
    return isAbsoluteUrl(cleanBase) ? normalizePathnameOnly(new URL(cleanBase, getBaseOrigin()).pathname || "") : normalizePathnameOnly(cleanBase);
  } catch {
    return "";
  }
}
...
export function joinUrl(base = "", path = "") {
  ...
  if (shouldAvoidDoubleBase(cleanBase, cleanPath)) {
    if (isAbsoluteUrl(cleanBase)) {
      try {
        return `${new URL(cleanBase, getBaseOrigin()).origin}/${cleanPath}`;
      } catch {}
    }
    ...
    for (const [key, value] of entries) {
      ...
      if (Array.isArray(value)) {
        for (const item of value) {
          if (item !== undefined && item !== null && item !== "") url.searchParams.append(cleanKey, String(item));
        }
        continue;
      }
...
export function buildUrl(pathOrBase = "", queryOrPath = null, maybeQuery = null) {
  ...
  if (typeof queryOrPath === "string") {
    return appendQueryToUrl(isAbsoluteUrl(queryOrPath) ? rewriteForbiddenFrontendApiUrl(queryOrPath) : joinUrl(first, queryOrPath), maybeQuery);
  }
...
export function hasValidToken(token = null) {
  ...
  return Boolean(value && !BAD_TOKEN_VALUES.includes(value.toLowerCase()) && !/[\s\r\n\t]/.test(value));
}
...
function apiBasePath() {
  ...
  try {
    return isAbsoluteUrl(apiBase) ? normalizeCanonicalPath(new URL(apiBase, getBaseOrigin()).pathname || "") : normalizeCanonicalPath(apiBase);
  } catch {
    return "";
  }
}
...
function publicApiPaths() {
  return unique([
    ...PUBLIC_API_PATHS,
    ...toArray(config?.auth?.publicApiPaths || config?.publicApiPaths || []),
  ]).filter((path) => !isMeEndpoint(path));
}

function privateApiPaths() {
  return unique([
    ...PRIVATE_API_PATHS,
    ...ME_PATHS,
    ...toArray(config?.auth?.privateApiPaths || config?.privateApiPaths || []),
  ]);
}
...
function isExplicitPrivateApiPath(path = "") {
  ...
  return privateApiPaths().some((privatePath) => {
    ...
    return apiPathMatches(normalized, current) ||
      apiPathMatches(withoutBase, current) ||
      apiPathMatches(normalized, currentWithoutBase) ||
      apiPathMatches(withoutBase, currentWithoutBase);
  });
}
...
function pickFrom(source = {}, groups = [], keys = []) {
  const direct = pick(source, keys);
  if (direct) return direct;

  for (const group of groups) {
    const value = pick(source?.[group], keys);
    if (value) return value;
  }
...
function hasIdentity(value = {}) {
  return isPlainObject(value) && USER_ID_KEYS.some((key) => Boolean(safeText(value[key], "")));
}

function looksLikeAuthEnvelope(value = {}) {
  return isPlainObject(value) && AUTH_ENVELOPE_KEYS.some((key) => hasOwn(value, key));
}
...
function unwrapUserPayload(payload = null) {
  ...
  for (const key of USER_WRAPPER_KEYS) {
    if (hasIdentity(payload[key])) return payload[key];
  }

  for (const group of ["data", "payload", "result", "auth", "session", "sessionData", "response", "body"]) {
    for (const key of USER_WRAPPER_KEYS) {
      if (hasIdentity(payload[group]?.[key])) return payload[group][key];
    }
  }
...
function isSafeAvatarUrl(url = "") {
  ...
  try {
    return ["http:", "https:", "blob:"].includes(new URL(value, getBaseOrigin()).protocol);
  } catch {
    return false;
  }
}
...
function sanitizeUserPayload(value = {}, depth = 0, seen = new WeakSet()) {
  ...
  for (const [key, item] of Object.entries(value).slice(0, 160)) {
    ...
    if (Array.isArray(item)) {
      out[key] = item.slice(0, 120).map((entry) => isPlainObject(entry) ? sanitizeUserPayload(entry, depth + 1, seen) : entry);
      continue;
    }
...
export function normalizeUser(user = null) {
  ...
  const rawName = pickFrom(source, ["profile", "raw"], ["name", "fullName", "full_name", "displayName", "display_name", "nombre"]) || pick(source, ["username"]) || email || id || "Usuario";
  ...
  const emailLower = (pick(source, ["emailLower", "email_lower"]) || email || "").toLowerCase() || null;
  ...
  return {
    ...
    fullName: pickFrom(source, ["profile", "raw"], ["fullName", "full_name", "name", "nombre", "displayName", "display_name"]) || rawName,
    full_name: pickFrom(source, ["profile", "raw"], ["full_name", "fullName", "name", "nombre", "displayName", "display_name"]) || rawName,
    displayName: pickFrom(source, ["profile", "raw"], ["name", "fullName", "full_name", "displayName", "display_name", "nombre"]) || rawName,
    email,
    emailLower,
...
export function isUsableUser(user = null) {
  ...
  return Boolean(normalized && normalized.active !== false && firstNonEmpty(normalized.id, normalized.userId, normalized.user_id, normalized.username, normalized.email));
}

export function getUserDisplayName(user = null) {
  const source = unwrapUserPayload(user);
  return pickFrom(source, ["profile", "raw"], ["name", "fullName", "full_name", "displayName", "display_name", "nombre"]) || pick(source, ["username", "email"]) || "Usuario";
}

export function getUserUsername(user = null) {
  const source = unwrapUserPayload(user);
  return sanitizeUsername(pickFrom(source, ["profile", "raw"], ["username", "userName", "user_name", "nick", "alias", "login", "slug"]) || pick(source, ["email", "id", "userId"]));
}

export function getUserAvatarUrl(user = null) {
  const source = unwrapUserPayload(user);
  const hasAvatar = source?.hasAvatar ?? source?.has_avatar ?? source?.profile?.hasAvatar ?? source?.profile?.has_avatar ?? source?.raw?.hasAvatar ?? source?.raw?.has_avatar;
  return hasAvatar === false ? "" : resolveAvatarCandidate(source || {});
}
...
export function getCurrentLocationCanonicalPath() {
  try {
    return isBrowser() ? normalizeCanonicalPath(getCurrentLocationPath()) : DEFAULT_ROUTE;
  } catch {
    return DEFAULT_ROUTE;
  }
}
...
export function isExternalHref(href = "") {
  const value = safeText(href, "");
  if (!value || isUnsafeHref(value) || !isAbsoluteUrl(value)) return false;
...
export async function runHookSeries(hooks = [], payload) {
  ...
  for (const hook of safeArray(hooks)) {
    ...
    try {
      ...
      if (config?.debug) {
        try { console.error(`[${config?.appName || "Onion"}] Error ejecutando hook`, cloneError(error)); } catch {}
      }
    }
  }
...
export function getThemeColor(theme = config?.defaultTheme) {
  return theme === "light" ? config?.ui?.themeColorLight || "#f4f7fb" : config?.ui?.themeColorDark || "#0a0c11";
}
...
export function createAbortTimeout(ms = config?.requestTimeout) {
  ...
  const timeoutId = setTimeout(() => {
    try { controller.abort("timeout"); } catch { try { controller.abort(); } catch {} }
  }, normalizedMs);
...
export function normalizeHeaders(headers = {}) {
  ...
  return source.reduce((acc, [key, value]) => {
    ...
    if (normalizedKey && value !== undefined && value !== null && value !== "") acc[normalizedKey] = value;
    return acc;
  }, {});
}
...
export function mergeAbortSignals(signals = []) {
  ...
  try {
    if (typeof AbortSignal !== "undefined" && typeof AbortSignal.any === "function") return AbortSignal.any(validSignals);
  } catch {}
  ...
  const abortFrom = (sourceSignal) => {
    ...
    try { controller.abort(sourceSignal?.reason || "aborted"); } catch { try { controller.abort(); } catch {} } finally { teardown(); }
  };
...
export function isAbortError(error) {
  ...
  return name === "aborterror" || code === "20" || code === "abort_err" || message.includes("aborted") || message.includes("abort");
}

export function isProbablyTimeoutError(error) {
  const text = `${error?.message || ""} ${error?.name || ""} ${error?.code || ""} ${error?.raw || ""} ${error?.reason || ""}`.toLowerCase();
  return error?.timeout === true || text.includes("timeout") || text.includes("etimedout") || text.includes("request_timeout") || text.includes("boot_timeout");
}
...
export function detectNetworkHints(url = "") {
  ...
  try {
    if (navigator.onLine === false) hints.push("El navegador parece estar offline.");
  } catch {}
