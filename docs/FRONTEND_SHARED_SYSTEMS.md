...
| Clave compacta de comparación (`normalizeKey`) | `src/core/presentation-text.js` | Quita separadores (`-`, `_`, espacios) y pasa a minúsculas conservando acentos: es la clave con la que el kernel compara cabeceras, rutas y nombres de configuración (`core`, `http`, `router`, `media`, shared público). Vive junto a `cleanText` porque ya está en los tres cierres de arranque. Contrato: `tools/presentation-text-contract.mjs`. |
| Clave slug de valores de dominio (`slugKey`) | `src/core/slug-key.js` | Minúsculas, sin acentos (NFD), espacios y guiones unidos con `_`, fuera todo salvo `\w`, `:` y `.`; la clave de estados, prioridades, roles y planes en las vistas. Módulo propio, fuera de los cierres de arranque. Antes se llamaba `normalizeKey` en 24 módulos, el mismo nombre que la clave compacta del kernel con otro comportamiento. Cuatro helpers de vista que todavía se llaman `normalizeKey` construyen un slug con política propia (`@` permitido en perfil técnico, `_` por caracteres no válidos en Facturas, `slice(0, 80)` en la base de Home) y 16 módulos llevan pipelines slug inline con otro nombre: ambas listas están en el contrato como cota superior para la siguiente unidad. Contrato: `tools/presentation-text-contract.mjs`. |
