...
const [app, router, login, enhancements, layout, chrome, variables, privateRuntime, entityIntentPreload] = await Promise.all([
...
assert.match(privateRuntime, /await initModule\(EntityOverlayUI, payload\)/);
assert.match(privateRuntime, /await initModule\(EntityIntentPreloadUI, payload\)/);
assert.match(privateRuntime, /destroyLoaded\(EntityIntentPreloadUI\)/);
assert.match(entityIntentPreload, /authenticated\(\)/);
assert.match(entityIntentPreload, /prefetchFacturaDetail/);
...
console.log("Private runtime/auth layout contract: PASS · document handoff · authenticated entity intent preload");
