...
debounceTimer=setTimeout(searchUsers,250,q);
...
`${API_BASE}/api/admin/search/users?q=${encodeURIComponent(q)}&mode=inactive`,
{
headers:{
...
const res = await fetch(
`${API_BASE}/api/auth/activate/admin`,
{
method:"POST",
headers:{
...
if(!res.ok || !json.ok){
showToast(json?.error || "No se pudo activar la cuenta","error");
return;
}
