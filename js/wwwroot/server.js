...
Onion.runCleanup = function(){
  ...
  if(Onion.state.abortControllers?.size){
    Onion.state.abortControllers.forEach((controller)=>{
      try{ controller.abort(); }catch{}
    });
    Onion.state.abortControllers.clear();
  }
};
...
Onion.fetch = async function(url, opt = {}){
  ...
  Onion.state.abortControllers.add(controller);
  ...
  try{
    ...
    Onion.state.abortControllers.delete(controller);
  }
};
